...
を満たせば、理論上は任意の安定極を \(A-HC\) に配置できる。これは状態フィードバック

$$
\dot{x}=(A-BK)x
$$

の極配置問題と双対であり、\((A,C)\) のオブザーバ設計は、\((A^T,C^T)\) に対する状態フィードバック設計として扱える。すなわち、一般的な数値計算では

$$
H=\operatorname{place}(A^T,C^T,\{p_i\})^T
$$

の形で求めることができる。MATLABであれば `place(A', C', poles)'`、Pythonであれば `scipy.signal.place_poles(A.T, C.T, poles).gain_matrix.T` に相当する。
