...
\boldsymbol{v}_s
+
G(\boldsymbol{i}_s-\hat{\boldsymbol{i}}_s)
```

補正に使う出力誤差は、測定できる一次電流と推定一次電流の差である。ゲイン行列 `G` は4行2列であり、論文ではd/q対称性を保つため、次の構造に制限している。論文中の表記は次である。
...
3. 設計パラメータ `k` を選ぶ。
4. 上式で `g1..g4` を計算する。
5. `G` を構成し、一次電流推定誤差 `i_s - hat{i}_s` をオブザーバに注入する。
...
#### 17.2.4 Kubota/Matsuse/Nakano 1993との関係
