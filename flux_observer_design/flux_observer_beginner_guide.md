...
| 方式B | 回転dq座標 | 一次磁束2成分、二次磁束2成分 | 文献5.3のalpha/beta極指定を参考に、$-\alpha\pm j\beta$ の共役極を指定し、今回の4状態実装ではSylvester方程式で $H$ を求める |
| 方式C | 推定ロータ磁束座標 | 推定一次電流2成分、推定ロータ磁束大きさ、推定角 | SLED 2023 Appendix Aの閉形式式を使う |

方式A/Bは同じ回転dq座標4状態オブザーバであり、違いは目標極の与え方である。方式Cは状態変数と座標の取り方が異なり、すべり周波数の計算式が $\hat{\psi}_{Rq}=0$ を保つためのオブザーバ構成そのものになっている。
...
一方で、オンラインで毎周期Sylvester方程式を解くと計算負荷が重い。組込み機器では、オフラインでゲインテーブルを作る、またはより軽い方式Cを検討するのが現実的である。

## 11. 方式B: alpha/betaで共役極を指定する設計

方式Bでは、d/q軸の回転性を意識して、減衰と回転を分けて指定する。ここで重要なのは、基論文5.3そのものが、この章で使うSylvester方程式を解いているという意味ではない点である。
...
という設計思想である。一方、本成果物では既存の回転dq座標4状態オブザーバを維持するため、その目標極を実現する計算法としてSylvester方程式を使っている。したがって、方式Bは「論文5.3の完全再現」ではなく、「5.3のalpha/beta極指定を4状態フルオーダ実装へ写した派生方式」である。
...
この $F_{\alpha\beta}$ を、今回の実装で使う9章のSylvester方程式に入れる。
...
方式Bは、極を「実部と虚部」で指定できるため、回転座標系の誤差設計として見通しがよい。ただし、これは基論文5.3の手順そのものではなく、本成果物側の4状態実装へ接続するための構成である。今回の実装では方式Aと同じくSylvester方程式を解くため、オンライン計算負荷は軽くならない。
...
方式Cでは、方式A/Bのような4行2列の一般行列 $H$ を毎周期計算しない。代わりに、閉形式の係数を使う。
...
方式A/Bでは、すべり周波数は外部から来る入力である。方式Cでは、すべり周波数はオブザーバ内部の拘束式から決まる。この違いを混同してはいけない。
...
8. 方式A/Bでは、Sylvester方程式 $TA-FT=GC$ を使って $H=T^{-1}G$ を求める。
9. 方式Cでは、推定ロータ磁束座標を使い、$\hat{\psi}_{Rq}=0$ の拘束から $\omega_s$、つまりすべり周波数相当の量が決まる。

方式A/Bは、状態空間と極配置の考え方を理解するのに適している。方式Cは、組込み実装の計算負荷を下げる実用的な方式として有力である。ただし、方式Cではオブザーバ構成とすべり周波数計算式が一体であるため、$\omega_s$ の式を単なる外部すべり推定式として扱ってはいけない。

## 参考文献

1. 堀孝正, ほか, 誘導機オブザーバ設計に関する文献5.3の極配置法。
