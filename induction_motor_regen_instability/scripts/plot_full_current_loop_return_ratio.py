...
OUT = Path("outputs/full_current_loop_return_ratio.png")
OUT_TXT = Path("outputs/full_current_loop_return_ratio.txt")
...
def slip_command(flux: np.ndarray, phihat: float, iq_ref: float, flux_source: str) -> float:
    if flux_source in ("id_ref", "id_feedback"):
        phi_for_slip = phihat
...
def decoupling_voltage(flux: np.ndarray, phihat: float, iq_ref: float, ff_mode: str, flux_source: str) -> np.ndarray:
    if ff_mode == "none":
        return np.zeros(2)
    id_ref = ID0
    slip = slip_command(flux, phihat, iq_ref, flux_source)
    omega_r = POLE_PAIRS * SPEED_RPM * 2.0 * math.pi / 60.0
    omega_e = omega_r + slip
    if ff_mode == "flux_feedback":
        phi_dec = flux[2]
    elif ff_mode == "command":
        phi_dec = LM * id_ref
    else:
...
def required_voltage(flux: np.ndarray, phihat: float, iq_ref: float, flux_source: str) -> np.ndarray:
...
def plant_rhs(x: np.ndarray, u_pi: np.ndarray, iq_ref: float, ff_mode: str, flux_source: str) -> np.ndarray:
    flux = x[:4]
    phihat = x[4]
    ...
    omega_r = POLE_PAIRS * SPEED_RPM * 2.0 * math.pi / 60.0
    omega_e = omega_r + slip
    v = u_pi + decoupling_voltage(flux, phihat, iq_ref, ff_mode, flux_source)
    psid, psiq, phid, phiq = flux
    dflux = np.array(
        [
...
def operating_point(iq_ref: float, ff_mode: str, flux_source: str) -> tuple[np.ndarray, np.ndarray]:
    flux = operating_flux(iq_ref)
    phihat = LM * ID0
    x0 = np.r_[flux, phihat]
    u0 = required_voltage(flux, phihat, iq_ref, flux_source) - decoupling_voltage(flux, phihat, iq_ref, ff_mode, flux_source)
    return x0, u0


def linearize_plant(iq_ref: float, ff_mode: str, flux_source: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    x0, u0 = operating_point(iq_ref, ff_mode, flux_source)
    n = len(x0)
    a = np.zeros((n, n))
    b = np.zeros((n, 2))
    c = np.zeros((2, n))
    for k in range(n):
        ...
        xp[k] += h
        xm[k] -= h
        a[:, k] = (plant_rhs(xp, u0, iq_ref, ff_mode, flux_source) - plant_rhs(xm, u0, iq_ref, ff_mode, flux_source)) / (2.0 * h)
    ...
    for k in range(2):
        ...
        up[k] += h
        um[k] -= h
        b[:, k] = (plant_rhs(x0, up, iq_ref, ff_mode, flux_source) - plant_rhs(x0, um, iq_ref, ff_mode, flux_source)) / (2.0 * h)
    return a, b, c
...
def loop_response(iq_ref: float, ff_mode: str, flux_source: str, omega: np.ndarray) -> np.ndarray:
    a, b, c = linearize_plant(iq_ref, ff_mode, flux_source)
    eye = np.eye(a.shape[0])
    loops = np.zeros((len(omega), 2, 2), dtype=complex)
    for i, w in enumerate(omega):
        plant = c @ np.linalg.solve(1j * w * eye - a, b)
        loops[i] = ci_response(np.array([w]))[0] * plant
    return loops
...
def main() -> None:
    ...
    w = 2.0 * math.pi * f
    cases = [
        ("id_ref slip", "id_ref", "#0072B2", "-"),
        ("id_feedback slip", "id_feedback", "#D55E00", "--"),
        ("actual flux slip", "actual_flux", "#009E73", "-."),
    ]
    fig, axes = plt.subplots(3, 2, figsize=(15, 11), sharex="col")
    fig.suptitle("MIMO current-loop return ratio with slip-flux source, 5000 r/min regen", fontsize=14, fontweight="bold")
    lines = []
    for label, source, color, ls in cases:
        loops = loop_response(-ID0, "flux_feedback", source, w)
        ...
        axes[0, 0].semilogx(f, 20 * np.log10(np.maximum(sigmas[:, 0], 1e-30)), color=color, ls=ls, lw=2.0, label=f"{label} sigma_max")
        axes[0, 0].semilogx(f, 20 * np.log10(np.maximum(sigmas[:, -1], 1e-30)), color=color, ls=ls, lw=1.2, alpha=0.55)
        axes[1, 0].semilogx(f, 20 * np.log10(np.maximum(np.abs(det_i_l), 1e-30)), color=color, ls=ls, lw=2.0, label=label)
        ...
        for mode in range(2):
            axes[0, 1].plot(eigvals[:, mode].real, eigvals[:, mode].imag, color=color, ls=ls, lw=1.6, alpha=0.8)
