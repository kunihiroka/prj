...
OUT = Path("outputs/full_current_loop_kp_sweep_idref.png")
OUT_TXT = Path("outputs/full_current_loop_kp_sweep_idref.txt")
...
def decoupling_voltage(flux: np.ndarray, phihat: float, iq_ref: float) -> np.ndarray:
    id_ref = ID0
...
def required_voltage(flux: np.ndarray, phihat: float, iq_ref: float) -> np.ndarray:
...
def plant_rhs(x: np.ndarray, u_pi: np.ndarray, iq_ref: float) -> np.ndarray:
    flux = x[:4]
    phihat = x[4]
    ...
    omega_r = POLE_PAIRS * SPEED_RPM * 2.0 * math.pi / 60.0
    omega_e = omega_r + slip
    v = u_pi + decoupling_voltage(flux, phihat, iq_ref)
    psid, psiq, phid, phiq = flux
    dflux = np.array(
        [
...
def operating_point(iq_ref: float) -> tuple[np.ndarray, np.ndarray]:
    flux = operating_flux(iq_ref)
    phihat = LM * ID0
    x0 = np.r_[flux, phihat]
    u0 = required_voltage(flux, phihat, iq_ref) - decoupling_voltage(flux, phihat, iq_ref)
    return x0, u0


def linearize_plant(iq_ref: float) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    x0, u0 = operating_point(iq_ref)
    n = len(x0)
    a = np.zeros((n, n))
    b = np.zeros((n, 2))
    c = np.zeros((2, n))
    for k in range(n):
        ...
        xp[k] += h
        xm[k] -= h
        a[:, k] = (plant_rhs(xp, u0, iq_ref) - plant_rhs(xm, u0, iq_ref)) / (2.0 * h)
    ...
    for k in range(2):
        ...
        up[k] += h
        um[k] -= h
        b[:, k] = (plant_rhs(x0, up, iq_ref) - plant_rhs(x0, um, iq_ref)) / (2.0 * h)
    return a, b, c


def ci_response(w: float, kp_scale: float) -> complex:
    kp = KP_BASE * kp_scale
    return kp * (1.0 + KI / (1j * w))


def loop_response(iq_ref: float, omega: np.ndarray, kp_scale: float) -> np.ndarray:
    a, b, c = linearize_plant(iq_ref)
    eye = np.eye(a.shape[0])
    loops = np.zeros((len(omega), 2, 2), dtype=complex)
    for i, w in enumerate(omega):
        plant = c @ np.linalg.solve(1j * w * eye - a, b)
        loops[i] = ci_response(w, kp_scale) * plant
    return loops
...
def main() -> None:
    scales = [0.0, 0.5, 1.0, 3.0]
    colors = ["#6b7280", "#0072B2", "#E69F00", "#D55E00"]
    labels = ["0%", "50%", "100%", "300%"]
    f = np.logspace(-1, 4, 1600)
    w = 2.0 * math.pi * f

    fig, axes = plt.subplots(3, 2, figsize=(15, 11), sharex="col")
    fig.suptitle("Kp sweep of MIMO current-loop return ratio, id_ref slip, 5000 r/min regen", fontsize=14, fontweight="bold")
    lines = []
    for scale, color, label in zip(scales, colors, labels):
        if scale == 0.0:
            det_i_l = np.ones_like(f, dtype=complex)
            sigmas = np.zeros((len(f), 2))
            eigvals = np.zeros((len(f), 2), dtype=complex)
        else:
            loops = loop_response(-ID0, w, scale)
        ...
        axes[0, 0].semilogx(f, 20 * np.log10(np.maximum(sigmas[:, 0], 1e-30)), color=color, lw=2.0, label=f"Kp {label} sigma_max")
        axes[0, 0].semilogx(f, 20 * np.log10(np.maximum(sigmas[:, -1], 1e-30)), color=color, lw=1.2, alpha=0.55)
        axes[1, 0].semilogx(f, 20 * np.log10(np.maximum(np.abs(det_i_l), 1e-30)), color=color, lw=2.0, label=f"Kp {label}")
        axes[2, 0].semilogx(f, phase_deg(det_i_l), color=color, lw=2.0, label=f"Kp {label}")
        axes[1, 1].plot(det_i_l.real, det_i_l.imag, color=color, lw=2.0, label=f"Kp {label}")
        for mode in range(2):
        ...
        lines.append(f"Kp {label}: min |det(I+L)|={dist:.4f} at {f[idx]:.3f} Hz, det={det_i_l[idx].real:+.4f}{det_i_l[idx].imag:+.4f}j")

    axes[0, 0].axhline(0, color="black", lw=0.8)
    axes[0, 0].set_title("Singular values of return ratio L_i")
    axes[0, 0].set_ylabel("gain [dB]")
    axes[0, 0].set_ylim(-80, 55)
